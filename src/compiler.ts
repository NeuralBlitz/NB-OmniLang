...
export class SimpleTokenizer {
  ...
  private isAtEnd() { return this.pos >= this.source.length; }
  private peek() { return this.source[this.pos] || ""; }
  private peekNext() { return this.source[this.pos + 1] || ""; }
  private advance() { return this.source[this.pos++]; }

  private isDigit(ch: string) { return /[0-9]/.test(ch); }
  private isAlpha(ch: string) { return /[a-zA-Z_$]/.test(ch); }
  private isAlphaNumeric(ch: string) { return /[a-zA-Z0-9_$]/.test(ch); }
...
export class SimpleParser {
  ...
  private peek() { return this.tokens[this.pos] || { type: "EOF", value: "", start: 0, end: 0 }; }
  private advance() { return this.tokens[this.pos++]; }
  private check(type: string, value?: string) {
    const tok = this.peek();
    if (value) return tok.type === type && tok.value === value;
    return tok.type === type;
  }
  private match(type: string, value?: string) {
  ...
  private parseStatement(): ASTNode | null {
    ...
    if (tok.value === "function") return this.parseFunction();
    if (tok.value === "const" || tok.value === "let" || tok.value === "var") return this.parseVarDecl();
    if (tok.value === "class") return this.parseClass();
  ...
  private parseClass(): ASTNode {
    ...
    const name = this.advance().value;
    let superClass = null;
    if (this.match("Keyword", "extends")) superClass = { type: "Identifier", name: this.advance().value };
    const body = this.parseBlock();
    return { type: "ClassDeclaration", name, superClass, body };
  }

  private parseBlockOrStmt(): ASTNode {
    if (this.match("Operator", "{")) return this.parseBlock();
  ...
  private parseParams(): string[] {
    ...
    while (!this.check("Operator", ")")) {
      if (params.length > 0) this.match("Operator", ",");
      if (this.match("Identifier")) params.push(this.advance().value);
    }
    return params;
  }
  ...
  private parseMember(): ASTNode {
    ...
    if (this.match("Keyword", "this")) return { type: "ThisExpression" };
    return this.parsePrimary();
  }

  private parsePrimary(): ASTNode {
    const tok = this.peek();
    if (this.match("Identifier")) return { type: "Identifier", name: tok.value };
    if (this.match("Numeric")) return { type: "NumericLiteral", value: Number(tok.value) };
    if (this.match("String")) return { type: "StringLiteral", value: tok.value };
    if (this.match("Keyword", "true") || this.match("Keyword", "false")) return { type: "BooleanLiteral", value: tok.value === "true" };
    if (this.match("Keyword", "null")) return { type: "NullLiteral", value: null };
    if (this.match("Operator", "(")) { const expr = this.parseExpression(); this.match("Operator", ")"); return expr; }
    if (this.match("Operator", "[")) { const elements = this.parseList(); return { type: "ArrayExpression", elements }; }
    if (this.match("Operator", "{")) { const props = this.parseObject(); return { type: "ObjectExpression", properties: props }; }
  ...
  private parseArgs(): any[] {
    const args: any[] = [];
    while (!this.check("Operator", ")")) {
      if (args.length > 0) this.match("Operator", ",");
  ...
  private parseList(): any[] {
    const elements: any[] = [];
    while (!this.check("Operator", "]")) {
      if (elements.length > 0) this.match("Operator", ",");
  ...
  private parseObject(): any[] {
    ...
    while (!this.check("Operator", "}")) {
      if (properties.length > 0) this.match("Operator", ",");
...
export class SimpleCompiler {
  compile(source: string): { code: string; ast: any[]; tokens: Token[] } {
  ...
  private emitNode(node: any): string {
    switch (node.type) {
      case "VariableDeclaration":
        const decls = node.declarations.map((d: any) => `${d.name}${d.init ? " = " + this.emitNode(d.init) : ""}`).join(", ");
      ...
      case "IfStatement":
        ...
        if (node.alternate) s += ` else ${this.emitStatement(node.alternate)}`;
        return s;
      case "WhileStatement":
        return `while (${this.emitNode(node.test)}) ${this.emitStatement(node.body)}`;
      case "ForStatement":
        const init = node.init ? (node.init.declarations ? `${node.init.kind} ${node.init.declarations.map((d: any) => d.name + (d.init ? " = " + this.emitNode(d.init) : "")).join(", ")}` : this.emitNode(node.init)) : "";
      ...
      case "CallExpression":
        const args = node.arguments.map((a: any) => this.emitNode(a)).join(", ");
        return `${this.emitNode(node.callee)}(${args})`;
      case "MemberExpression":
        if (node.computed) return `${this.emitNode(node.object)}[${this.emitNode(node.property)}]`;
      ...
      case "ArrayExpression":
        return "[" + node.elements.map((e: any) => this.emitNode(e)).join(", ") + "]";
      case "ObjectExpression":
        return "{ " + node.properties.map((p: any) => `${p.key}: ${this.emitNode(p.value)}`).join(", ") + " }";
      default:
        return "";
    }
  }
  ...
  private emitStatement(node: any): string {
    if (node.type === "BlockStatement") return this.emitBlock(node.body);
    return this.emitNode(node) + ";";
  }

  private emitBlock(body: any[]): string {
