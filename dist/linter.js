export class Linter {
    ...
    lintFence(fence, definedNames, dataNames, content) {
        const issues = [];
        if (fence.type === "data") {
            if (!fence.attrs.name) {
    ...
    checkCircularDependencies(fences, dataNames, computedNames) {
        const deps = new Map();
        for (const fence of fences) {
            if (fence.type === "compute" && fence.attrs.name) {
                const name = fence.attrs.name;
                const fenceDeps = new Set();
                const dataRefRegex = /data\.(\w+)/g;
                let match;
                while ((match = dataRefRegex.exec(fence.content)) !== null) {
