...
program
    ...
    .option("--allowed-domain <domain>", "Allowed domain for fetch (can be repeated)", (val, list) => { list.push(val); return list; }, [])
    ...
    try {
        ...
        if (verbose)
            console.log("Parsing OmniLang document...");
        ...
        if (format === "json") {
            ...
            result = omni.toHtml({ theme: theme, csp });
        }
        if (verbose)
        ...
        console.error("Error building document:", error instanceof Error ? error.message : String(error));
    ...
    const build = async () => {
        ...
        buildTimeout = setTimeout(async () => {
            try {
                ...
                omni.parse(content);
                await omni.execute();
                const result = omni.toHtml({ theme: theme, csp: options.csp ?? false });
                fs.writeFileSync(output, result, "utf-8");
...
`;
    fs.writeFileSync(path.join(projectDir, "examples", "example.omd"), exampleContent);
